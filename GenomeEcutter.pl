...
# Get total bases used
$size=`wc ./GenomeEcutter/$file.temp`;
@size=split(" ",$size);
close FILE;
close TMP1;
open (TMP1,"./GenomeEcutter/$file.temp") or die "Sorry, you don't have the permisison for writing files!\n";
open (TMP01,">./GenomeEcutter/$file.SizeSelected.temp") or die "Sorry, you don't have the permisison for writing files!\n";
foreach my $Tline(<TMP1>)
{
	print TMP01 $Tline if length($Tline) >=$opts{Min}
}
my $size2=`wc ./GenomeEcutter/$file.SizeSelected.temp`;
my @size2=split(" ",$size2);
close TMP1;
close TMP01;
undef $line;
open (TMP2,">./GenomeEcutter/$file.single") or die "Sorry, you don't have the permisison for writing files!\n";
print TMP2 "Enzyme","\t","TotalBasesInput(Mb)","\t","TotalBasesUsed(Mb)","\t","EnzymeCuttingSite","\t","#ofCuttingSites","\t","#ofNormalized/kb","\t","MeanLength","\t","100bp-","\t","100-200bp","\t","200-400bp","\t","400-600bp","\t","600-800bp","\t","800-1000bp","\t","1000bp+","\n";
...
{	
	...
	$sum=0;
	$cutsite{$enz[0]}=$enz[2];
	open(TMP4,"./GenomeEcutter/$file.SizeSelected.temp") or die "can't find temp file containing sequence!\n";
	my $num=0;	
	foreach $temps1(<TMP4>)
	{
		my $int=0;
	...
	if($num>0)
	{	
		print TMP2 $enz[0],"\t",$size[2]/1000000,"\t",$size2[2]/1000000,"\t",$enz[2],"\t",$count,"\t",$count*1000/$size2[2],"\t",$sum/$num,"\t",join("\t",@Sinterval),"\n";
	}
	else
	{
		print TMP2 $enz[0],"\t",$size[2]/1000000,"\t",$size2[2]/1000000,"\t",$enz[2],"\t",0,"\t",0,"\t",0,"\t",join("\t",@Sinterval),"\n";
	}
...
unlink "./GenomeEcutter/$file.SizeSelected.temp";
...
print TMP3 "EnzymePair","\t","TotalBasesInput(Mb)","\t","TotalBasesUsed(Mb)","\t","CutSiteI","\t","CutSiteII","\t","MeanInterval","\t","100bp-","\t","100-200bp","\t","200-400bp","\t","400-600bp","\t","600-800bp","\t","800-1000bp","\t","1000bp+","\t","Total\n";
@name=sort @name;
my @name2=@name;
...
{	
	...
	foreach $key2(@name2)
	{
		...
		if($key ne $key2 && $cutsite{$key}!~/$cutsite{$key2}/ && $cutsite{$key2}!~/$cutsite{$key}/)
		{
			...
			print TMP3 $key,"-",$key2,"\t",$size[2]/1000000,"\t",$size2[2]/1000000,"\t",$cutsite{$key},"\t",$cutsite{$key2},"\t",join("\t",@double),"\n"; 
			undef @temp;
			undef @temp1;
			undef @a1;
			undef @a2;
